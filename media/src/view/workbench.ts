 ...
 * 作者看不见自己正在改的细纲写了什么、这一场的「必须发生」有哪几条，只能去开
 ...
 * 标题栏还在。侧边栏本来就只有三百来像素宽，一份七小节的场景卡能吃掉半屏对话。
 *
 ...
 * 复制（「必须发生」是要抄进正文的），所以鼠标必须进得来，收起要留宽限期——
 ...
 * 轮对话，同时对着这一场的必须发生」变得不可能，而那正是钉住要解决的场景。
 *
 ...
 *
 * 悬停版够用于「瞥一眼这一场要发生什么」，但照着「必须发生」写正文时鼠标要回
...
export function installWorkbench(): void {
  ...
  // 点一下钉住：照着「必须发生」写正文时鼠标要回输入框，那一刻浮窗得留得住。
  // 再点一次收起——同一个入口，同一个键。
