...
| [tiers.ts](tiers.ts) | ★ **模型分档**：三档（快速 / 均衡 / 精标）与八项后台任务的归属。纯数据 + 纯函数（`tierOf` / `refsForTask` / `describeTaskModels`），无 I/O 也无 Node 依赖——所以设置页可以直接 import 同一份标签与默认映射，界面上写的和跑起来的必然一致。 |
