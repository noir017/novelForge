...
- **章节** —— 每层内倒序排列（最新的在上）。行首圆点表示摘要新鲜度：`●` 最新、`○` 缺失或过期。点标题打开正文，行上有「在此续写」「总结」「看摘要」。
...
分组标题栏和每个文件夹行的右侧（悬停显出）有两个新建按钮：**＋** 在这里新建一章/一张角色卡/一条设定，**🗀** 在这里新建子文件夹。每一行——文件也好文件夹也好——还有三个操作：

| 图标 | 操作 | 说明 |
|---|---|---|
| ✎ | 重命名 | 章节的 `NNN-` 序号前缀自动保留，只让你改标题部分。正文首行的 `# 标题` 只在它原本与文件名一致时跟着改——你要是单独改过它，那是你手写的东西，改名不动 |
| ↦ | 移动到… | 弹出同一个区里的目录列表（含根目录）选落点 |
| 🗑 | 删除 | 搬进 `.novelforge/.trash/`，保留原来的相对路径，随时能手动捞回来。文件夹连里面的东西一起搬，删之前会告诉你里面有多少个文件 |
