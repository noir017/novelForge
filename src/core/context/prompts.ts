 ...
 * 四个阶段该给出完全不同的回答：策划编辑去动故事结构，剧情编剧去调这一段的
...
const STAGE_DUTY: Record<CreationStage, string> = {
  ...
  // 从前它叫「章节细纲」，要求给出这一章的开头与结尾画面——那既抢了场景层的活，
  // 又逼着每一段强行自成起讫，而长篇小说的剧情本来就是连续的。
  plot:
    '你负责把故事往前推：这一段发生什么、事件之间的因果、冲突在哪一步翻转、' +
    '谁付出什么代价、最后收在什么局面上。\n' +
    '**你交出的是剧情脉络，不是场景。** 不要写具体的画面、天气、动作细节或台词——' +
    '那是下一层的事；也不要规定这一段从哪句话开头、到哪句话结尾，那是写正文时才定的东西。\n' +
    '你也不必让这一段自成起讫：它是一条连续剧情里的一截，前面接着上一段的局面，' +
    '后面留给下一段。作者最后会自己把正文切成发布用的章节，那与你无关。',
  scene:
    '你负责把这一幕**想具体**：站在什么地方、什么天气什么光线、人物做了哪些动作、' +
    '说了哪些话、哪些细节值得写进去。你产出的是写正文时能直接取用的素材，' +
    '不是对正文的规定——你不写正文，也不给写正文的人立规矩。',
  manuscript:
    '你负责把已经定好的剧情写成文学文本。剧情走向不由你决定——那在本段剧情里已经定了。' +
...
const CAPABILITY_TASK: Record<Capability, string> = {
  ...
  settle:
    '你和作者刚讨论完这一段（完整对话就在上面）。把讨论中**已经达成的结论**整理成产物。\n' +
...
export function buildSystemPrompt(action: CreationAction, config: NovelConfig, targetWords?: number): string {
  ...
  if (stage === 'manuscript' && (capability === 'generate' || capability === 'rewrite')) {
    const lines = [
      ...
      '3. 完整覆盖本段剧情纲要中的每一个要点，按纲要顺序推进，并把它扩写成有场景、有对白、有细节的成稿。',
      '4. 不要复述前情，不要写「上回说到」，直接从上一段结尾的情境自然接续。',
 ...
 * 与单段摘要同一套——模型不听话是常态，而解析失败等于这一次生成白花钱。
 */
  ...
  switch (stage) {
    case 'outline':
      return capability === 'split'
        ? [
            '请把上面的大纲拆成一段一段的剧情，只输出 JSON，不要有任何其它文字：',
            '',
            '```json',
            ...
            '```',
            '',
            '每段是一个完整的事件或一次转折，展开后约三千字正文的体量——不必对齐章节，',
            '作者最后会自己把正文切成发布用的章节。',
    ...
    case 'plot':
      if (capability === 'split') {
        return [
          '请把这一段剧情拆成若干场景，只输出 JSON，不要有任何其它文字：',
          '',
          '```json',
          ...
          '```',
          '',
          '一段通常 3~6 场。每一场都要有自己的目的与一个可以失败的点；',
      ...
      return [
        ...
        '最后一环写清这一段收在什么局面上（谁在哪、关系变成什么样、谁知道了什么），下一段从那里接。',
        '**不要写具体画面、天气、动作细节或台词**，那些在场景层。也不要规定这一段的开头与结尾。',
        '「冲突与转折」说清主冲突是什么、在哪一步翻转、谁付出什么代价。',
        '「伏笔与回收」分别写清这一段埋下什么、兑现了前面哪一处。',
        '想不清楚的小节留空字符串，不要为了填满而编。' +
          (targetWords ? `\n这一段目标篇幅约 ${targetWords} 字，据此把握事件密度。` : ''),
      ].join('\n');
    ...
    case 'scene':
      return [
        ...
        '',
        '各节写什么：',
        '- 「目的」一句话说清这一幕在本段里担什么用，供写正文的人定位，不必展开。',
