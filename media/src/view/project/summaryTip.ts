...
const summaryPending = new Set<number>();

let hoverTimer: ReturnType<typeof setTimeout> | null = null;
/** 收起的宽限计时。与 hoverTimer 是两件事，别合并。 */
let closeTimer: ReturnType<typeof setTimeout> | null = null;
/** 当前浮窗。 */
let hoverTip: { order: number; box: HTMLElement } | null = null;
/** 正在等延迟的那一行（延迟到点时用它定位）。 */
...
function scheduleSummaryTip(row: HTMLElement): void {
  ...
  // 浮窗永远弹不出来。
  if (hoverTip?.order === order || hoverRow === row) {
    return;
  }
  hideSummaryTip();
  hoverRow = row;
...
function showSummaryTip(row: HTMLElement, order: number): void {
  const box = mk('div', 'summary-tip');
  box.appendChild(buildBody(summaryCache.get(order)));
...
function buildBody(view?: ChapterSummaryView): DocumentFragment {
  ...
  head.appendChild(mk('span', 'summary-tip-title', `第 ${view.order} 章 ${view.title}`.trim()));
  ...
  if (!view.exists) {
    frag.appendChild(mk('div', 'hint', '这一章还没有摘要。右键「总结本章」可以生成。'));
    return frag;
  }
...
export function applySummary(view: ChapterSummaryView): void {
  summaryPending.delete(view.order);
  summaryCache.set(view.order, view);
  if (hoverTip?.order !== view.order) {
    return;
  }
...
export function installSummaryTip(): void {
  ...
  el.projectBody.addEventListener('mouseover', (e) => {
    const row = closestFrom<HTMLElement>(e.target, '.row-chapter');
    if (row) {
