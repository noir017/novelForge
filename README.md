...
四个可折叠分组（摘要里出现过还没建卡的人物另成一组，见下）。行上只有内容——图标、名字、字数/副标题——**所有操作都在右键菜单里**，和文件管理器一样：

- **章节** —— 每层内倒序排列（最新的在上）。行首圆点表示摘要新鲜度：`●` 最新、`○` 缺失或过期；字数后面的「· 草稿」表示这一章已经有草稿。点标题打开正文；右键有「打开 / 在此续写 / 打开草稿 / 总结本章 / 看摘要」，以及重命名、移动、删除。
