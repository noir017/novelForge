...
import { Chapter, ChapterSummary } from '../model/types';

/** 章节 relPath → 该章摘要（没总结过的章节不在表里）。 */
export type SummaryIndex = Map<string, ChapterSummary>;

/**
 * 读全书摘要。
 *
 * 章节列表由调用方传入：工程页那条路上它早就读过了，再读一次等于把
 * 全书正文又扫一遍（`listChapters` 会读每章正文算字数与 hash）。
 */
...
export async function summaryOf(
  ...
  return index ? index.get(chapter.relPath) : project.readSummary(chapter);
}
