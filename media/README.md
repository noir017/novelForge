...
- **摘要浮窗按需取、事件委托**（`view/project/summaryTip.ts`）：鼠标停在**剧情行**上约半秒后弹出这一段的摘要（章节行是纯文件行，没有摘要可言）。摘要正文**不在** `ProjectTree` 里（那棵树每次文件变动都全量重推，塞进去等于每保存一次就推几百 KB），悬停时发 `requestSummary{plotRelPath}` 单段去要。**缓存只在收到 `project` 消息时清**——那说明磁盘变过；折叠文件夹走 `rerenderProject()` 不经那条分支，缓存留着。监听用事件委托挂在 `#projectBody` 上（行每次重渲染都换掉，逐行 `addEventListener` 会堆积），浮窗与右键菜单同样是挂在 `body` 上的 `position: fixed`。
...
- **与「工程」页是两件事**：工程页是按语义整理过的视图（剧情段带流水线状态与摘要新鲜度、章节是纯文件列表、角色带别名），只看得见几个可管理区里的文件；这一页是磁盘上真实的目录结构，一个都不藏。`.novelforge/` 里的摘要、会话、`project.json` 只有这里进得去——这就是它存在的理由，别为了「干净」把点开头的目录过滤掉（`fileTree.ts` 的 `HIDDEN_DIR_NAMES` 只挡 `node_modules` 与 `.git`）。
