...
1. **开发调试**：按 `F5` 启动 Extension Development Host 时自动打开这里，直接可以试写细纲 / 拆场景 / 写正文 / 拆成章节 / 摘要 / 角色卡。
...
│   ├── 002-客栈里的女人.md
│   └── 003-夜访.md
├── scenes/                  场景素材，按细纲名建目录（<细纲名>/NN-场景名.md）
...
```
outline.md → plots/NNN.md → scenes/NNN/ → manuscripts/NNN.md →（拆分）→ chapters/NNN.md → summaries/NNN.md
```
