...
| [identity.ts](identity.ts) | ★ 同一人聚类（纯函数）：把摘要里散落的称呼归并成人。判据是**同段共现作硬约束的贪心聚类**——同一段 cast 里各自出场的两个称呼永不合并，候选链接按「多少段这么写过」计票贪心处理。朴素并查集在这里是错的：一条幻觉别名会顺着传递闭包把主角和她孪生弟弟并成一个人。 |
| [pipeline.ts](pipeline.ts) | ★ **创作流水线的领域模型**：`Stage × Capability × Target` 三元组（阶段是 大纲/剧情/细节/正文）、每阶段的身份与可用能力（`settle` 只有剧情层有）、`CreationTarget` 与它的稳定字符串键、剧情段流水线状态的推导（`deriveStage` / `deriveProgress`）与全书状态机（`deriveBookStage` / `deriveBookNextStep`），以及界面直接吃的两样——`commandsFor`（`/` 命令面板的命令表）与 `deriveNextStep`（状态机 → 主按钮上那一个动作，判据与 `deriveStage` 同源）。**纯类型 + 纯函数，零 import**，所以前端可以直接 import 同一份表。它继续留在 `model/`；读磁盘的聚合器是 [`../views/pipeline.ts`](../views/pipeline.ts)，不要混为一处。 |
| [plotFile.ts](plotFile.ts) | ★ 剧情细纲（`.novelforge/plots/NNN-标题.md`）的格式：**四个固定小节**（目标 / 剧情脉络 / 冲突与转折 / 伏笔与回收）+ frontmatter 的 `upstreamHash`，段号来自文件名前缀。`isPlotFilled` **只认「剧情脉络」**——只写了目标等于什么都没排，流水线该停在剧情层。纯函数无 I/O。**只认 `.md`**。 |
| [sceneFile.ts](sceneFile.ts) | 场景卡（`.novelforge/scenes/<段名>/NN-标题.md`）的格式：六个固定小节（目的 / 环境 / 人物状态 / 动作 / 对话 / 细节与意象）+ 场号来自文件名前缀。**这一层产的是正文可以直接取用的素材，不是对正文的约束**——该发生什么、不该发生什么归上一层的剧情管。纯函数无 I/O。**只认 `.md`**——它是插件自己的数据格式，与「章节不认扩展名」相反。 |
...
| [project.ts](project.ts) | ★ `NovelProject`：数据访问层，所有 read*/write* 都在这里。含初始化模板、剧情段索引（manifest v2）、场景/正文/摘要三套按段名镜像的路径推导、草稿按章节路径镜像、摘要与场景的新鲜度指纹（`beatsHashFor`）、`.novel` → `.novelforge` 迁移检测，以及各区目录的**递归扫描**。 |
...
- **三套伴生文件，一条规则**：场景目录、正文、摘要都按**剧情段文件名的词干**镜像到各自的目录下（`plots/012-夜入.md` → `scenes/012-夜入/`、`manuscripts/012-夜入.md`、`summaries/012-夜入.md`），不落任何索引——没有索引就没有会漂移的第二份真相。改标题会改文件名，所以 `writePlot` 自己把三套一并搬过去（`carryPlotCompanions`），`deletePlot` 一并搬进 `.trash/`；少一套就会出现「界面上说这一段还没拆场景，而那几个场景躺在旁边一个孤儿目录里」。目标已存在时**不动**（不静默覆盖）。草稿是另一条线：它镜像的是**章节**在章节根之下的相对路径（`fileOps.ts` 的 `carryDraft`），因为草稿属于发布区。
- **新建只出序号名**：`writePlot` / `createChapter` 的 `title` 允许为空，那时落成 `007.md` 且不写 H1——走流水线新建一段时还没有标题可言（它是排完剧情才定得下来的东西），凭空塞一个「第7段」进文件名只是个假标题，而它会进上下文。为此有个 `safeStem`：**空标题给空串，不给 `sanitizeFileName` 的兜底名「未命名」**，否则会得到 `007-未命名.md`。标题回落链（`extractH1` → 文件名词干 → `第 N 段`）本来就认这种名；界面上的说法统一走 `pipeline.ts` 的 `plotLabel`，未命名时只报序号，不写成「第 7 段《第 7 段》」。写 H1 时用的是**清洗后的词干**而不是原样 `title`，两者一致，改名时 `renamedBody` 才认得出「这个 H1 是跟着文件名走的」。
- **正文带 frontmatter，发布章节不带**：`manuscripts/*.md` 是插件自己的产物，一定是 `.md`，所以「这份正文依据的是哪一版场景」那个 `beatsHash` 就写在它自己的 frontmatter 里（manifest 因此只剩全书级信息）。`chapters/` 下的文件可以是 `.txt` / 无扩展名 / `.json`，是作者的东西，插件一个字节都不往里写。`readManuscript` 的 `contentHash` **只哈希正文本身**（剥掉 frontmatter 与 H1）——写一次 `beatsHash` 不该让摘要立刻过期。
...
- **场景的 `characters` 不进出场统计**：frontmatter 里那份是**计划出场**，摘要里的 `cast` 才是**实际出场**。混进 `castIndex` 会污染出场段落统计与角色卡语料（AGENTS.md 第 14 条）。这条看起来很诱人，写在 `sceneFile.ts` 的文件头是为了别让人手滑。
- **角色/设定的 slug 是路径**：根目录下的文件 slug 就是文件名（与改造前一致），子目录里的形如 `主角/林昭`——上下文明细里的 `character:<slug>` 因此仍然唯一。
- **摘要新鲜度**：正文写盘后重算 `contentHash`，与摘要 frontmatter 里的 `sourceHash` 比对，不一致即过期。**还没写正文的段不算过期**——那不是「摘要旧了」，是还没到总结那一步，混进来会让同步摘要的确认框报一个虚高的调用次数。摘要不自动生成，只提示。`syncManifest` 按 `file` 匹配不上时会按段号兜底，因此给一段改标题（文件名跟着变）不会丢掉「已总结」的记录。
...
- **角色卡的 `appearsIn` / `updatedThrough` 是缓存**：前者是该角色出场的段号，后者是上次「更新角色卡」读到了第几段（增量更新的依据）。真相永远是各段摘要，落在卡里只为两件事——不读全部摘要就能在角色页显示「出场 12 段」，以及日后按人物检索段落。
