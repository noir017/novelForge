...
| `novel.models` | 第一个可用 | **默认模型列表**（有序）。工程页任务串行时用第一个，失败随机换用其余；并发时轮转负载均衡 |
| `novel.model` | 列表第一项 | 当前模型引用，如 `glm/glm-4-plus`。恒等于 `novel.models[0]`，兼容保留 |
| `novel.concurrency` | `3` | 工程页批量任务的并发请求数，1 表示串行 |
| `novel.fallbackAttempts` | `2` | 一次调用失败后换模型重试的次数上限，0 表示不重试 |
...
设置页的「默认模型」是一份**排好序的清单**，不是单选。它只管工程页的后台任务——同步摘要、重建全书摘要、更新/批量更新角色卡、批量建卡、生成设定、提取文风、提取角色：

- **串行时**（`novel.concurrency = 1`，或本来就只有一件事要做）恒用第一个模型；报错就在**其余模型里随机挑一个**重试，最多 `novel.fallbackAttempts` 次。同一个模型不会被试两遍，列表里只有一个模型时不重试。
