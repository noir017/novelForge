...
- **工作区卡是一只悬停浮窗**（`view/workbench.ts`）：流水线条上留一个一行高的入口（`#workbench`），悬停浮出这一层的产物，**点一下钉住**（照着「必须发生」写正文时鼠标要回输入框），Esc / 再点一次 / 点别处收起。从前它是消息流顶部一张 `position: sticky` 的卡片——滚不出视野，代价是永远占着消息流最上面那一块，而且**既关不掉也藏不起来**（只能折叠成一条标题栏，那条标题栏还在），侧边栏只有三百来像素宽，一份七小节的场景卡能吃掉半屏对话。改成浮窗之后 `renderSession` 不必再把卡片节点放回 `#messages` 了。
  ...
  - **不监听 scroll**，这是它与另外三只浮窗唯一的不同：那三只的目标是工程页里会滚动的行，页面一滚就得收；这个入口长在不滚的流水线条上（`.pipeline` 是 `flex: 0 0 auto`），消息流滚动时它一动不动。照抄那条会让「滚上去看前一轮对话，同时对着这一场的必须发生」变得不可能——而那正是钉住要解决的场景。
