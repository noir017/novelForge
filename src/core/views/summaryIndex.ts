 ...
 * （[cast.ts](cast.ts)）。三处各自 `readSummary` 的话，一次工程页刷新就把每段的摘要
 * 读三遍——五百段工程即一千五百次多余的小文件读，而工程页每次保存都刷新。
 *
...
import { PlotSummary } from '../model/types';

/** 剧情段 relPath → 该段摘要（没总结过的段不在表里）。 */
export type SummaryIndex = Map<string, PlotSummary>;

/**
 * 读全书摘要。
 *
 * 剧情段列表由调用方传入：工程页那条路上它早就读过了，再读一遍等于把
 * `plots/` 整个又扫一次（`listPlots` 会逐段读文件解析四个小节）。
 */
...
export async function summaryOf(
  ...
  return index ? index.get(plotRelPath) : project.readSummary(plotRelPath);
}
