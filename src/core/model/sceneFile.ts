/**
 * 场景文件（`.novelforge/scenes/<剧情段名>/NN-标题.md`）的格式定义。
 *
 * **纯函数、零 I/O**，与 chapterFile.ts 同构：什么文件算场景、序号怎么来、
 * 怎么解析、怎么渲染，只在这里定义一次。
 *
 * 场景是「剧情 → 正文」之间缺的那一层。剧情段说「林昭翻墙进入青云宗」，
 ...
 * 2. **约束该在上一层。** 该发生什么、不该发生什么是**剧情段**的事（这一段
 ...
 *
 * 「目的」是唯一一节**不是素材**的：它由拆场景那一步从剧情段带过来，只用来
 * 定位这一幕在本段的位置。判 ready 时因此把它排除在外，见 {@link isSceneReady}。
 */
 ...
 * 就把剧情段里的一句话填进去了，每一场一出生就带着它——拿它当判据的话，
