...
- **要动磁盘之前那一句问固定在输入框上方**（`view/gate.ts`）：`gate` 到了就往 `#gateDock`（`.composer` 的第一格，紧贴输入框上沿）挂一张卡片——要做什么、动的是哪个文件、这一次的参数（折叠着），加上按钮；点下去发回 `gateResult`。**两种问法共用这一张卡**：agent 动手前的闸门，以及**产物落盘前那一句**（后者从前是气泡末尾那颗「采纳写入」——一颗可以永远不点的按钮；单步创作那条路的落盘卡片只有两颗按钮，没有「停止 agent」，靠 `msg.stop` 在不在判断）。**不复用 `providerModal` 那个遮罩层**：作者要判断的上下文（它前几步读了什么、正要写哪个文件）就在被遮罩盖住的消息流里，而且模态框期间连翻一眼那个文件都做不到。**也不再挂在气泡上**：那样它会跟着内容滚——agent 随后还在说话、还在调工具，几行之后卡片就滚出视野了，而循环正卡在它上面等回答，界面看起来像「卡住不动」。按钮上的字全部来自消息（工具自报的说辞，前端不写死），同意贴最右、「停止 agent」摆最左（它掐的是整轮，与另外两颗不是同类）。答完卡片就撤（后端回话之前不撤的话，三颗都点得着，而只有第一下算数），消息流里补一行 `.gate-note` 当记录——紧跟着的工具条只说「未执行」，「因为我按了跳过」只有那一行讲得出。那一行不进 `SerializedTurn`：这一轮结束时气泡按会话重建，它自然消失，决定本身已经记在工具条上了。
