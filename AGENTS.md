...
| `src/core/agent/` | ★ 多步：对话循环、工具注册表、状态注入、预算闸门与无进展检测、**策略与确认闸门**（`policy.ts`）。七个工具：读三件 + `generate` + `write` / `edit` / `run`，**没有删除/改名/移动**；「下一步该做什么」由 `deriveNextStep` 每回合注入，agent 拿着它去执行而不是另做判断 | [src/core/agent/README.md](src/core/agent/README.md) |
