...
| `src/core/agent/` | ★ 多步：对话循环、工具注册表、状态注入、预算闸门与无进展检测。**三期只注册只读四件套**（`list` / `read` / `search` / `generate`），一个字都不写磁盘；「下一步该做什么」由 `deriveNextStep` 每回合注入，agent 拿着它去执行而不是另做判断 | [src/core/agent/README.md](src/core/agent/README.md) |
...
24. **agent 是调度者，不是第二个作者**：`core/agent/` 的循环只做「拿着工具达成一个目标」，创作质量仍然来自 `context/recipes.ts` + `context/prompts.ts` 的分阶段装配——`generate` 工具内部那次调用走的就是它们，**领域知识（「剧情不写画面天气台词」）只在 prompts.ts 里写一份**，工具描述里再写一遍会白烧 token 且两处迟早跑偏。四条配套约束：**(a) 生成产物绝不回灌 agent 上下文**——`generate` 只回 draftId 与形状摘要，正文流给前端气泡；三千字正文塞回循环，每走一步重烧一遍。**(b) `generate` 的 `history` 恒为空**——agent 的工具调用不是作者的讨论，混进装配器会被当成创作要求（所以 `settle` 在 agent 里明确不支持，它要沉淀的那段讨论 agent 手上没有）。**(c) 无进展检测**——连续两次同工具同参数就提示、三次就停；模型卡在一个读不到的路径上反复重试是最常见的烧钱方式。**(d) 触顶不静默停**——预算触顶 / 上下文压不下 / 原地打转，都要给模型最后一轮（摘掉 tools）说明「做到哪了、还差什么」，直接掐断的话作者只看到一段没头没尾的输出。写盘工具（`write` / `edit` / `run`）四期才加，三期的 agent 一个字都写不进磁盘。
