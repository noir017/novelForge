/**
 * `manuscript` handler：中转站里的正文。
 *
 * **落在 `manuscripts/`，不是 `chapters/`。** 那里是作者切好的发布章节；
 * 正文先落在中转站，什么时候切、怎么切由他自己定（第 23 条）。
 *
 * 三件事：
 *
 * 1. **渲染**：`Artifact{kind:'manuscript'}` → 就是那段正文。首次写入时
 *    补 frontmatter 与 `# 第N章… · 正文` 标题行。
 * 2. **追加是默认**：一段正文可以分几次写，顺序拼起来才是完整的一段。
 *    两次追加之间插一行 `---`——那是**默认的拆分候选点**，一次写作的边界
 *    正是最可能的章节边界；给一个能改的默认，比让作者从头自己标要好。
 * 3. **记账**：`upstreamHash = plotContentHash(plot)`——正文的上游就是它那一段
 *    的细纲。少了这一步，这一段会永远显示「正文与剧情对不上」或永远不显示。
 *
 * ## 上游从场景换成了细纲
 *
 * 从前这一格叫 `beatsHash`，上游是那一段的**场景集合**（`scenes/`）。场景那一层
 * 已经删掉（见 `model/pipeline.ts` 的文件头），于是链少一环：
 * `plots/X.md ──hash──▶ manuscripts/X.md`。名字随之统一成 `upstreamHash`，
 * 与卷纲、细纲两层用同一个词；`readManuscript` 两个名字都认，老工程的正文
 * 不会因为改名而变成「手写的」。
 *
 * ## `contentHash` 只哈希正文本身（第 18b 条）
 *
 * 不含 frontmatter 与标题行——写一次 `upstreamHash` 不该让摘要立刻过期。
 * 那个哈希在 `project.readManuscript` 里算，这里只负责别去污染正文。
 */
import * as fs from 'node:fs/promises';
import { rewriteFrontmatter, stringifyFrontmatter } from '../../model/markdown';
import { NovelProject } from '../../model/project';
import { plotContentHash } from '../../views/pipeline';
import { Handler, HandlerCtx } from './types';

export const manuscriptHandler: Handler = {
  async render(ctx: HandlerCtx, artifact) {
    if (artifact.kind !== 'manuscript') {
      throw new Error(`「${ctx.rel}」不接 ${artifact.kind} 产物`);
    }
    return artifact.text;
  },

  /** 首次写入时的 frontmatter + `# 第N章… · 正文` 标题行。 */
  async appendHead(ctx: HandlerCtx) {
    return ctx.path.plotRelPath ? manuscriptHead(ctx.project, ctx.path.plotRelPath) : '';
  },

  /**
   * 两次追加之间那一行 `---` 是**默认的拆分候选点**（第 23 条）：
   * 一段正文分几次写，每次写作的边界正是最可能的章节边界。给一个能改的默认，
   * 比让作者从头自己标要好——他可以删掉、也可以另加。
   */
  appendSeparator: '\n\n---\n\n',

  /**
   * 记账：把这一段细纲当前的指纹落进正文的 frontmatter。
   *
   * `rewriteFrontmatter` 只改 `---` 之间那一段，**正文一个字节不动**。
   * 没有 frontmatter 就是作者自己贴进来的正文——**不给它补一个**，
   * 从没记录过指纹的正文永不标脏（第 18a 条）。
   */
  async after(ctx: HandlerCtx, text: string) {
    const plotRelPath = ctx.path.plotRelPath;
    if (!plotRelPath) {
      return [];
    }
    const upstream = await manuscriptUpstreamHash(ctx.project, plotRelPath);
    if (!upstream) {
      return [];
    }
    const next = rewriteFrontmatter(text, { upstreamHash: upstream });
    if (next === undefined || next === text) {
      return [];
    }
    await fs.writeFile(ctx.project.pathOf(ctx.rel), next, 'utf8');
    return [`记下剧情指纹 ${upstream}`];
  },
};

/**
 * 这一段正文的上游指纹：它那份细纲的内容 hash。细纲不在时给空串（不标脏）。
 *
 * `plotContentHash` 只哈希四个小节、不含 frontmatter（第 18b 条）：
 * `upstreamHash` 自己就在细纲的 frontmatter 里，算进去会让「排一次剧情」
 * 立刻使正文过期。哈希在 `views/pipeline.ts` 里定义一次。
 */
export async function manuscriptUpstreamHash(
  project: NovelProject,
  plotRelPath: string
): Promise<string> {
  const plot = await project.readPlot(plotRelPath);
  return plot ? plotContentHash(plot) : '';
}

/** 首次写入时的 frontmatter + 标题行。第二次之后走追加，不再重复。 */
export async function manuscriptHead(
  project: NovelProject,
  plotRelPath: string
): Promise<string> {
  const plot = await project.readPlot(plotRelPath);
  const fm = stringifyFrontmatter({ plot: plotRelPath, generatedBy: 'novel-forge' });
  const heading = `# 第${plot?.no ?? 0}章${plot?.title ? ` ${plot.title}` : ''} · 正文`;
  return `${fm}\n\n${heading}\n\n`;
}
