...
| `src/core/` | 核心逻辑层入口（含协议 `protocol/`、文件能力 `files/`、只读聚合与界面快照 `views/`、日志 logger.ts、失败记录 errorLog.ts、工程库 db.ts、长任务 progress.ts）。`views/pipeline.ts` 是磁盘 I/O 聚合器；`model/pipeline.ts` 仍是纯领域模型与状态机，绝不搬进 `views/`。 | [src/core/README.md](src/core/README.md) |
