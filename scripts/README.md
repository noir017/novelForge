...
十六个不依赖 VS Code、也不需要真实 API Key 的测试脚本，`npm run smoke` 依次执行（末尾的 `smoke-server.js` 需要 bun），`npm test` = typecheck + core 纯度检查 + smoke。
...
| [smoke-cast.js](smoke-cast.js) | ★ **「谁是谁」**：别名的泛称过滤（代词/亲属称谓/泛称/描述短语全丢，`方老魔女`、`店小二` 这类专属称呼与「以泛称当正式名」的角色不受影响）；同一人聚类——`方源` / `古月方源` 交替出现归成一个人并合并出场章节，而**同章共现的两人绝不合并**（一条幻觉别名把孪生姐弟连起来时必须拦下并说得出理由）、两个重要角色之间单票不够两票才合、泛称别名不把两个人串起来；出场索引——正式名压过别名（别人的章节抢不走）、互指别名进 `conflicts`；维护命令——清理别名只改 frontmatter 而正文一个字节不动，合并重复卡（认出重复组、绝不把两个人列成一组、被合并的卡搬进回收站、保留卡吸收别名且「已读到」水位线退回第一章没读过的之前、名字包含只算弱提示）；`rewriteFrontmatter` 的定点改写 |
...
| [smoke-errorlog.js](smoke-errorlog.js) | ★ **工程库与失败记录**。驱动适配层：开库建表、写读往返、`detail` 可为 NULL、连接按工程根缓存，以及**关库之后删得掉目录**（语句不 finalize 的话 Windows 上库文件占着，所有用临时工程的 smoke 都会 EBUSY）。纯读取不建库——查失败记录/清记录/读日志历史都不该在作者的 `.novelforge/` 里凭空生出 db 文件，只有写入方才建。失败记录的生命周期：按 relPath 索引、同一目标+同一动作只留最新一条、不同动作各留各的、清某个动作不误伤别的、一个目标最多显示 3 条，以及**失败记录进得了工程页快照**。日志持久化：落盘往返、**挂 sink 之前的缓冲会被补写**（启动那段才不会丢）且补写不产生重复、历史条目的 seq 取负不与实时序号撞、攒批不丢条、`limit` 与翻页、退订后不再写库。**库不可用时全线静默降级**：用一个目录占住库文件的位置，验证一个 API 都不抛、工程页照常构建、且开库失败只 warn 一次（不是每次调用都重试刷屏） |
...
- 数据目录用 `sample-novel/` 或临时目录，测试后自清理（对 `sample-novel` 只读——`smoke.js` 有 hash 断言，写入类用例一律另开临时工程）。**碰过工程库的脚本收尾时必须先 `db.resetDatabases()` 再删目录**：SQLite 连接开着时 Windows 上删不掉 `.novelforge/novelforge.db`，临时工程会全留在 temp 里（EBUSY）。
