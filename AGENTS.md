...
- **工具层与 agent 层互不缠绕**：能对工程做什么（[src/core/tools/](src/core/tools/README.md)）与谁拿着它做事（[src/core/agent/](src/core/agent/README.md)）是两层。工具不认识预算（只 `usage.record(n)` 报数，上限与「已用 3/10」那句话在调用方）、不认识闸门（只自报 `ToolIntent` 的五档性质与说辞，弹不弹框是宿主的事）；循环不认识 `Workspace` / `DraftStore` / 任何一个具体工具（手上只有一个 `ToolInvoker`），绑环境的是 `controller/agent.ts`。方向由 [tests/contract/layerBoundary.test.js](tests/contract/layerBoundary.test.js) 守着：`tools/` 一行都不 import `agent/`，`agent/` 只 `import type` 那一份契约。两个理由：**工具将来要能端出去做 MCP**（形状照 `tools/list` + `tools/call` 摆的，差的传输/会话/无 UI 时怎么确认三件记在 tools 那份 README 里），**循环要能换成更轻的实现**。
...
3. **不静默覆盖**：角色卡更新走 diff 确认；style.md 覆盖前先问；作者在落盘卡片上点头之前正文只存在会话里；类文件操作遇到同名目标一律报错退出；内置编辑器保存走内容 hash 乐观锁（[src/core/files/fileEditing.ts](src/core/files/fileEditing.ts)），磁盘变过就报冲突让用户取舍。
...
8. **段号与章号是两条轴，界面上的「剧情 N」是推导出来的**：章节顺序永远由文件名数字前缀决定，与所在目录层级无关；分卷不重置编号。细纲（`.novelforge/plots/<卷词干>/NNN-段名.md`）**按卷分子目录**，段的归属只看目录、不落 frontmatter（目录已经说了，再记一份就会漂移）；根下直接放的段是合法的「未分卷」，老工程全长这样。**段号只是 `plots/` 里的排序键**——一段可以拆成三章，所以 `chapters/` 那一侧自己连续排号（`nextChapterNo()`），而界面上那个「剧情 N」是 `segmentDisplayNo(最新章号, 位次)` 推出来的。`nextPlotNo()` 仍跨两个目录取最大号 +1，好让新建的段在文件名上排在最后。**拆分不再顺延后面的段号**（从前一段拆成三章要把后面几十份细纲整体改名，连带搬走各自的场景目录与中转站正文——一次重命名风暴，只为维持「细纲与章同号」这条已经不存在的不变量）。「这一段交付到了哪几章」显式记在细纲的 frontmatter（`chapters:`）里，那是「段 → 章」唯一的链；老工程没有这个字段，那时**只对 `plots/` 根下的段**退回同号判定（`views/pipeline.ts` 的 `chaptersOfSegment`，判据只写一次）。摘要的轴是**章节**（`summaries/` 镜像 `chapters/` 下的相对路径），上下文装配的正文也优先取 `chapters/`，还没拆分的才回落到中转站。草稿按章节在章节根之下的相对路径镜像存放，文件名（含扩展名）原样沿用。**场景与中转站正文镜像段在 `plots/` 之下的整段路径**（`plots/01-觉醒/007-入宗.md` → `scenes/01-觉醒/007-入宗/`、`manuscripts/01-觉醒/007-入宗.md`），未分卷的段镜像出来自然就是扁的，与本层出现之前完全一致——老工程一个文件都不用搬。
...
18. **上下游新鲜度只靠 hash 传播，不调模型**：产物串成一条指纹链——`outline.md` →（卷纲 frontmatter 的 `upstreamHash`）→ `volumes/*.md` →（细纲 frontmatter 的 `upstreamHash`）→ `plots/V/*.md` →（场景 frontmatter 的 `upstreamHash`）→ `scenes/X/*.md` →（正文 frontmatter 的 `beatsHash`）→ `manuscripts/X.md` →（**拆分**）→ `chapters/X.md` →（摘要 frontmatter 的 `sourceHash`）→ `summaries/X.md`。改了大纲，所有卷纲标脏；改了某一卷，**那一卷的**剧情段标脏（从前是改大纲让全书每一段都标脏——粒度太粗，改一句立意换来一屏 ⟳）；改了某一段的细纲，该段场景标脏；改了场景，中转站那份正文标脏。**拆分是这条链上唯一的人工闸口**：中转站那份拆完就删了，所以**已发布的章不会被拉回「待写正文」**（`deriveStage` 先看 `chapterExists`，成品在就短路整条生产链）——把作者已经发出去的文字标成待写是在撺掇他重写；工程页那一行仍会挂 ⟳ 提醒，够了。**代价是零次模型调用、零幻觉、零 token**。把「变更影响」做成 AI 功能，等于每改一行剧情就烧一次钱，而且会给出看起来很像但没有依据的影响清单。三条配套约束：**(a) 手写的产物永不标脏**——`upstreamHash` 为空或 `beatsHash` 从没记录过，说明它不是这条链生出来的，拿凭空的过期标记催作者重做，他会学会无视所有标记；只有「记录过一次、现在对不上」才算变更。**(b) 指纹只哈希内容，不哈希状态**——`beatsHashFor` 排除场景的 `status`（采纳正文时会把场景标成 `written`，那一次写入不该让刚写好的正文立刻显示「上游已变更」），`plotContentHash` 只哈希四个小节、不含 frontmatter，`readManuscript` 的 `contentHash` 只哈希正文本身、不含 frontmatter 与标题行（写一次 `beatsHash` 不该让摘要立刻过期）。**(c) 流水线状态一律从磁盘推导，绝不落盘**（[src/core/model/pipeline.ts](src/core/model/pipeline.ts) 的 `deriveStage`）——存一个 `status: writing` 字段的话，作者手删半段正文之后它就在撒谎，而字数与 hash 永远诚实。
19. **产物落盘前必须过一遍人，而且是当场过**：创作页的四层产物（大纲、剧情、场景卡、正文）与摘要/角色卡一样，`generate` 只把文本交回界面，作者在**对话里那张权限卡片**上点了「写入」才落盘（[src/core/controller/gate.ts](src/core/controller/gate.ts) → [generation/accept.ts](src/core/generation/accept.ts)），且目标已有内容时先走 `reviewReplace`（插件开 diff，独立版弹确认）。**这一问与 agent 的策略无关**（三种模式都问），也**不做成一颗按钮**：从前气泡末尾那颗「采纳写入」可以拖到第二天再点，于是这条承诺在界面上是一件可以无限拖延的事，而 agent 早接着往下做了；卡片当场问、当场答，答完把结论回给模型（不然它会再生成一遍）。落盘的是**气泡里当下那份文本**——作者可能先改两个字再点写入。**批量路径反过来：一律跳过已有产物的目标，不问、不覆盖**——一次弹几十个 diff 没有人看得完，跳过是那条路上唯一安全的做法（[src/core/features/pipelineBatch.ts](src/core/features/pipelineBatch.ts) 的三条：批量写剧情 / 批量拆场景 / 批量写正文）。同理，批量路径的解析用不带全文兜底的 `parsePlotStrict`：兜底会把模型的一句「我不太确定这一章写什么」变成一份「已排好」的细纲，紧接着的批量拆场景还会照着它往下拆；创作页保留兜底，因为那里产物摊在屏幕上，用户看得见它是什么。**没有 artifact 的回复根本不问**——落点由后端算（`describeArtifactOf`），前端猜不出一段讨论该写到哪一层。
20. **界面永远只推荐一个下一步，且由状态机算出来**：创作页的主按钮来自 `deriveNextStep`（[src/core/model/pipeline.ts](src/core/model/pipeline.ts)），与 `deriveStage` **共用同一套判据**——两处各判各的，界面上就会出现「徽章说待拆场景，按钮让你写正文」。全书那一层同理走 `deriveBookStage` / `deriveBookNextStep`（四档：没大纲 → 写大纲；有大纲没卷 → 拆卷；有卷没段 → 去第一卷拆段；有段或已经有章 → 交给按段的流水线），不在 controller 里手写。**agent 也不例外**：它每回合从 `core/agent/context.ts` 的 `buildStateBrief` 免费拿到同一份结论（label 与 hint 逐字照搬），拿着去执行而不是自己判断——所以 agent 没有 `status` 工具，也不该有。同理，选中一章走的是 `selectPlot` 而不是前端自己拼 target：只有后端知道那一章处于什么状态。**`selectPlot` 按路径认，不按号猜**——三种形状：一份真实的细纲路径（剧情段那一行）、一个已发布的章（那时找**它的来源段**，即 frontmatter 里记着这一章的那一段）、一份**并不存在**的细纲路径（`plotPathForNo` 算出来的，老工程的章走这条，按文件名里的号找同号的章）。**绝不在两条轴之间按号互认**：段号与章号是两条轴，猜出来的那个段会是毫不相干的一段。四条配套约束：**(a) 其余命令收进 `/` 面板，不平铺**——一排等重的按钮看不出该点哪个，而任何一刻真正要按的只有一个。**(b) 命令表只有一份**（`commandsFor`，前端直接打包那个零 import 的纯函数模块），前端自己抄一份就会出现后端不认的命令，点了什么都不发生。**(c) 做完了就不给下一步**——造一个假的「下一步」等于逼作者一直有事可做，而写完就是写完了（对 agent 尤其要紧：给它一个假的下一步，它会自作主张挑一章开始烧钱）。**(d) 命令的字打在输入框里，界面显示与发给模型的内容分开**——`/` 是输入框里的普通字符，面板只是浮在上方的候选列表（判据、过滤串全从输入框的值算，见 [media/README.md](media/README.md)）；命令名记在 `ChatTurn.command` 上供气泡显示，**绝不塞进 `content`**——那句话会被当成作者的要求装进 prompt，与旧界面逼他手打一句「请生成细纲」是同一个毛病。
21. **细纲是剧情脉络，不是场景**：`plots/**/*.md` 四节——`目标` / `剧情脉络` / `冲突与转折` / `伏笔与回收`，主体是剧情脉络（[src/core/model/plotFile.ts](src/core/model/plotFile.ts) 的 `PLOT_SECTION_KEYS`）。卷纲（[src/core/model/volumeFile.ts](src/core/model/volumeFile.ts)）另有四节——`目标` / `剧情走向` / `关键转折` / `伏笔与回收`，**刻意与细纲不同名**：两层混用同一组小节名会让装配出来的上下文里两层长得一模一样，模型分不清自己在写哪一层。**不写画面、天气、动作细节或台词**（那是场景层的职责），也**不规定这一段从哪开头、到哪结尾**——这正是「不按章硬切」那半边的落点：写多长由剧情决定，正文出来后作者会自己切成发布章，模型不必为了凑一章而强行收束。提示词里那几句禁令（[src/core/context/prompts.ts](src/core/context/prompts.ts) 的 `STAGE_DUTY.plot` 与 `buildOutputContract`）是这条约束的落点，别在加需求时把它们稀释掉。`isPlotFilled` 只认「剧情脉络」：只写了目标等于什么都没排，流水线该停在剧情层。
...
23. **拆分是作者的活，工具不猜断点也不起名**：中转站正文拆成发布章走 [src/core/features/splitChapter.ts](src/core/features/splitChapter.ts)，按正文里**单独一行 `---`**（`chapterFile.ts` 的 `splitByMark`）切。**零次模型调用**：第一章沿用原标题，其余落成纯序号名（`101.md`）等作者自己改——让模型给章起名，是拿 token 换一批作者八成要重写的名字。写正文时每次追加之间自动插一行 `---` 当**默认候选**（场景边界最可能就是章节边界），作者删改即可。章号接在**现有最后一章**之后（`nextChapterNo()`），与这一段的段号无关；**一个别的文件都不动**——后面还没拆的段不改名、不挪场景目录（见第 8 条）。拆完把落点记进这一段的 frontmatter（`chapters:`）。
24. **agent 是调度者，不是第二个作者**：`core/agent/` 的循环只做「拿着工具达成一个目标」，创作质量仍然来自 `context/recipes.ts` + `context/prompts.ts` 的分阶段装配——`generate` 工具内部那次调用走的就是它们，**领域知识（「剧情不写画面天气台词」）只在 prompts.ts 里写一份**，工具描述里再写一遍会白烧 token 且两处迟早跑偏。四条配套约束：**(a) 生成产物绝不回灌 agent 上下文**——`generate` 只回 draftId 与形状摘要，正文流给前端气泡；三千字正文塞回循环，每走一步重烧一遍。**(b) `generate` 的 `history` 恒为空**——agent 的工具调用不是作者的讨论，混进装配器会被当成创作要求（所以 `settle` 在 agent 里明确不支持，它要沉淀的那段讨论 agent 手上没有）。**(c) 无进展检测**——连续两次同工具同参数就提示、三次就停；模型卡在一个读不到的路径上反复重试是最常见的烧钱方式。**(d) 触顶不静默停**——预算触顶 / 上下文压不下 / 原地打转 / 作者叫停，都要给模型最后一轮（摘掉 tools）说明「做到哪了、还差什么」，直接掐断的话作者只看到一段没头没尾的输出。

25. **agent 不越过既有的闸门**：它的写入走的是与落盘卡片**同一条** `workspace.write`——覆盖已有内容一律先审阅（插件开 diff，独立版弹确认框），批量动作照弹既有的确认框（含「预计调用 N 次」，那个数字由 feature 自己算一次，再经 `usage.record` 报给调用方记账；弹窗写 7 次、账上记 1 次正是第 4 条要防的事）。**给 agent 一条绕过闸门的快路，等于把前面 24 条一次性作废**，所以这一层没有任何新的保护代码：八条守卫在 `workspace/guard.ts`，[`core/agent/policy.ts`](src/core/agent/policy.ts) 只决定「动手之前要不要先问一句」（性质与说辞由工具自报 `ToolIntent`，policy 不认识任何一个工具名）。三条配套约束：**(a) 策略管不着保护**——三种模式（谨慎 / 默认 / 放手）里，「覆盖走审阅」与「批量动作的确认框」完全一样且不可配置；`edit` 改的也是已有内容而 `ws.edit` 不走 diff，所以它在三种模式下都弹确认框，框里写出 old → new 两段原文，那就是它的 diff。**(b) 拒绝要有信息量**——确认框给两个选项（确认 / 跳过；叫停整轮是输入框旁边那颗「停止」，不摆在闸门里，否则常被误当成「跳过」，而两者的后果差着一整轮），回给模型的话必须写明「作者跳过了这一步，磁盘上什么都没变，**不要重试同一个动作**」；只回一句「被拒绝」，它会把同一个动作再发一遍，每一次都是一整轮上下文的钱。关掉对话框当**停止**处理——他被问「要不要动你的磁盘」而没有回答，不该替他答「继续」。**(c) 不给删除 / 改名 / 移动**——`workspace` 上有 `remove` / `move`，但不暴露给 agent：收益接近零（作者要删东西会自己删），而细纲改名会连带搬走场景目录与中转站正文，删除即使进了 `.trash/` 作者也未必知道它删过什么。同理没有 `bash`、没有工程根之外的路径、没有裸 `fs`。

26. **思考深度是会话的属性，只作用于作者选定的那个模型**：对话页输入框旁那个下拉框（不思考 / 浅 / 中 / 深 / 极限，表在 [src/core/model/thinking.ts](src/core/model/thinking.ts)）落在 `ChatSession.thinking` 上，跟着会话走而不是进设置页——想多深是「这件事有多难」的函数，而排一卷的走向与改一个错别字往往就是相邻的两个会话。**缺省是「不思考」**，也就是这个开关出现之前的行为（第 4 条：思考 token 按输出计费，一句话可能因此贵好几倍，作者主动选了才开）。三条配套约束：**(a) 只有对话页的单次生成与 agent 循环带它**，工程页的后台批量任务一律不带——与第 12 条同源，那一档模型是作者按成本挑的，替他把七十六章的摘要都升级成深思考，账单上看不出是谁决定的；**(b) 一个档位、两套字段**（Responses 的 `reasoning.effort` 与 Anthropic 的 `output_config.effort` / 老模型的 `thinking.budget_tokens`），映射只在 `thinking.ts` 与两个 provider 里各写一次，界面上永远只有一个「极限」——作者选的是「想到底」，不是某家的枚举值；**(c) 上游拒了就换写法，不把这一轮判死**——`effort` 的梯子与 Anthropic 的两代写法都是按模型不同的，provider 按「接口地址 + 模型」把结论记在内存里（降一档 → 换一代写法 → 退到不带思考字段），每种模型一生只吃一次 400。**结论不写配置**：那是上游的事实，不是作者的偏好。
