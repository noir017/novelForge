...
| 文件 | 职责 |
|---|---|
| [creation.ts](creation.ts) | ★ `CreationSession`：创作编排——装配上下文 → 流式生成 → **由用户点了采纳才落盘**。`acceptArtifact` 按 target 分派到六条落盘路径（大纲/拆剧情段/剧情细纲/拆场景/场景卡/正文）。含 `preview()`（只装配不调用模型）、取消、输出清洗与连接测试。 |
...
| [pipelineBatch.ts](pipelineBatch.ts) | ★ 工程页的三条批量流水线动作：给缺剧情的段批量写剧情、给已有剧情的段批量拆场景、给场景齐了的段批量写正文。**只补不改**，走 runTask + runPool，失败挂在那一段上。 |
| [summarize.ts](summarize.ts) | 单段摘要编排（从 `manuscripts/` 的正文生成；解析、批量同步、全书 map-reduce）。系统提示词在 [summarizePrompt.ts](summarizePrompt.ts)。 |
| [summarizePrompt.ts](summarizePrompt.ts) | 单段摘要 / 阶段摘要 / 全书摘要三条系统提示。 |
...
| 解析失败 | 全文兜底（产物摊在屏幕上，用户看得见它是什么） | **不兜底**（`parsePlotStrict`）——没人逐份过目，兜底会把「这次失败了」变成「这一段已排好」，紧接着的批量拆场景还会照着它往下拆 |
| 出错 | 报错，用户重来 | 记进 errorLog 挂在那一段上，**继续跑完剩下的** |

两条路共用同一个 `buildContext`，因此批量与单次产出的是同一个质量。

## 摘要走 JSON

单段摘要的提示词要求模型输出 JSON（六个小节 + `出场人物: [{name, aliases}]`），`parseSummaryResponse` 解析成 `SummaryData`。落盘仍是 Markdown（作者要翻、要手改），结构化的出场人物额外写进 frontmatter 的 `cast`。

解析是**三层降级**，一层都不能少——模型不听话是常态，而解析失败意味着这一段的剧情永远进不了上下文：
...
| 作用对象 | 一批段落里的**所有**角色 | **一个**角色（另有按卡并发的批量版） |
| 段落从哪来 | 作者手输序号列表 | 摘要索引自动给出该角色的出场段落 |
| 上下文 | 一次装完（超预算就截断并 warn） | 按预算分批，逐批精炼同一张卡 |
| 入口 | 工具栏「提取/更新角色卡」 | 角色行右键；角色分组右键的批量更新；「未建卡」分组右键的「全部建卡」 |

`characterCard.ts` 的三条关键设计：

- **自动关联出场段落**：来自 [../views/cast.ts](../views/cast.ts) 的索引，作者不必再手输段号。
...
- **无先后依赖的条目并发跑**：段落摘要之间、角色卡之间、全书摘要的各阶段批次之间都没有依赖，一律经 [../runtime/concurrency.ts](../runtime/concurrency.ts) 的 `runPool`（并发量取 `config.concurrency`）。**有依赖的绝不并发**——同一张角色卡内部的分批必须串行，后一批要看到前一批的产出；全书摘要的 reduce 合并要等全部 map 到齐。并发下 `current` 只在项结束时 +1，`message` 报「已完成 n/N + 正在跑哪几项」。
...
- **超预算截断必须打 `warn`**：单段正文、角色提取的语料、文风样文、阶段摘要合并这四处都会按输入预算 `takeHead`。截断本身是对的，但作者选了五段却只读进两段半时必须说出来——这是「不静默截断」在本层的落法。
