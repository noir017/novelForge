...
- **摘要浮窗按需取、事件委托**（`view/project/summaryTip.ts`）：鼠标停在**章节行**上约半秒后弹出这一章的摘要（还没拆分发布的章没有摘要，浮窗会照实说）。摘要正文**不在** `ProjectTree` 里（那棵树每次文件变动都全量重推，塞进去等于每保存一次就推几百 KB），悬停时发 `requestSummary{plotRelPath}` 单章去要。**缓存只在收到 `project` 消息时清**——那说明磁盘变过；折叠文件夹走 `rerenderProject()` 不经那条分支，缓存留着。监听用事件委托挂在 `#projectBody` 上（行每次重渲染都换掉，逐行 `addEventListener` 会堆积），浮窗与右键菜单同样是挂在 `body` 上的 `position: fixed`。
...
- **创作页只推荐一个动作，其余走 `/`**（`view/pipeline.ts` + `view/commands.ts`）：主按钮来自后端状态机算出的 `next`，**点了就跑**，输入框可留空——「写剧情」本来就不需要作者再说什么。其余命令收在 `/` 面板里——**只有会写文件的**（生成/拆分，剧情层多一条 `/落定剧情`）；讨论不是命令，打字就是。挑中变成输入框里的一枚待执行 chip，用完即清。命令表来自 core 的 `commandsFor`（零 import 的纯函数，前端直接打包），**前端不自己维护一份**——否则界面上会出现后端不认的命令，点了什么都不发生。
...
- **命令类消息的气泡不能是空的**（`view/messages.ts` 的 `fillUserBody`）：命令（写剧情、拆出剧情段）一律不要求输入，作者一个字都不必打，于是 user 轮的 `content` 是空串。气泡里画一枚 `/写剧情` 标签（数据来自 `SerializedTurn.command`，后端记的是**按阶段具体化过的标签**），有补充要求就跟在下面。**命令名不写进 `content`**——那句话会被当成作者的要求装进 prompt，与旧界面逼他手打一句「请生成细纲」是同一个毛病。assistant 那一支**刻意保持成纯文本节点**：流式增量走 `body.textContent += delta`，里面有子元素的话第一片增量就会把它们冲掉。
- **agent 一轮里说的话与做的事按发生顺序交替**（`view/messages.ts` 的 `buildSegments`，实时那条路在 `view/index.ts`）：画的是 `SerializedTurn.segments`——一段文字一块 `.msg-body[data-seg=text]`，一次调用一条 `.tool-row`，**相邻的调用并进同一个 `.tools` 串**（连着五次 list/read 仍然是一串流水账，散成五块反而更乱），文字与 generate 卡打断它。实时那条路就地追加、绝不重建气泡：`delta` 接在**末尾那块**文字上（末尾不是文字就新开一块，那就是交替本身），`toolCall` 接在**末尾那一串**上（中间说过一句话就另起一串），插入位置一律取 `segmentAnchor()`（花销 / 上下文明细 / 行内动作之前）。从前是「所有工具挤在正文上方 + 所有话灌进同一个 `.msg-body`」，作者看不出哪句话是在哪一步之后说的。**有段的那一轮不给就地编辑**：`editTurn` 换的是整轮内容，多块之间没法映射；单步创作那条路（一块正文）照旧可改完再采纳。一轮刚开始那块空正文是给「它在想」留的位（推理模型常常先想几十秒），第一段是工具调用时由 `dropEmptyText()` 就地撤掉。
- **`generate` 产出的正文自成一张卡**（`view/messages.ts` 的 `buildGenCard`）：它走的是 `toolDelta` 而不是 `delta`（后端按 callId 贴的标签），流进 `.gen[data-call=…] .gen-body`，**只在卡内滚**——那几千字正在往下涨，让它把消息流一路往下顶的话，作者连上面刚说的那句话都看不成了。默认展开、限高 260px：`read` 回来的是它查资料的过程，而这是**产物本身**，作者接着就要判断「这份要不要落盘」。头两行（在做什么 / 结论如何）长在 `<summary>` 里，所以收起来之后还在，折叠只藏正文。`toolResult` 到了要**把卡里当下那份正文带过去**——它是一段段攒起来的，那条消息里根本没有它（落盘的结论随后还会重推一次同样的 `toolResult`，那时同样不能抹掉）。
- **要动磁盘之前那一句问固定在输入框上方**（`view/gate.ts`）：`gate` 到了就往 `#gateDock`（`.composer` 的第一格，紧贴输入框上沿）挂一张卡片——要做什么、动的是哪个文件、这一次的参数（折叠着），加上按钮；点下去发回 `gateResult`。**两种问法共用这一张卡**：agent 动手前的闸门，以及**产物落盘前那一句**（后者从前是气泡末尾那颗「采纳写入」——一颗可以永远不点的按钮；它的拒绝那颗写「不采纳」而不是「跳过」，答完那一行也补在正文**下面**，靠 `msg.callId` 在不在判断）。**不复用 `providerModal` 那个遮罩层**：作者要判断的上下文（它前几步读了什么、正要写哪个文件）就在被遮罩盖住的消息流里，而且模态框期间连翻一眼那个文件都做不到。**也不再挂在气泡上**：那样它会跟着内容滚——agent 随后还在说话、还在调工具，几行之后卡片就滚出视野了，而循环正卡在它上面等回答，界面看起来像「卡住不动」。**只有两颗按钮**——叫停整轮是输入框旁边那颗「停止」，不在这张卡上（摆进闸门只会被误当成「跳过」，而两者的后果差着一整轮）。按钮上的字全部来自消息（前端不写死），同意贴最右（离「发送」最近的那一侧就是「继续」），拒绝挨着它压成次级按钮。答完卡片就撤（后端回话之前不撤的话，两颗都点得着，而只有第一下算数），消息流里补一行 `.gate-note` 当记录——紧跟着的工具条只说「未执行」，「因为我按了跳过」只有那一行讲得出。那一行不进 `SerializedTurn`：这一轮结束时气泡按会话重建，它自然消失，决定本身已经记在工具条上了。
- **工作区卡是一只悬停浮窗**（`view/workbench.ts`）：流水线条上留一个一行高的入口（`#workbench`），悬停浮出这一层的产物，**点一下钉住**（照着细纲写正文时鼠标要回输入框），Esc / 再点一次 / 点别处收起。从前它是消息流顶部一张 `position: sticky` 的卡片——滚不出视野，代价是永远占着消息流最上面那一块，而且**既关不掉也藏不起来**（只能折叠成一条标题栏，那条标题栏还在），侧边栏只有三百来像素宽，一份四小节的细纲能吃掉半屏对话。改成浮窗之后 `renderSession` 不必再把卡片节点放回 `#messages` 了。
  - **类名用 `wbt-` 而不是 `wb-`**：`wb-` 是**独立版工作台外壳**的命名空间（`css/standalone/workbench.css` 的 `.wb-title` 是那条 38px 高的标题栏、`.wb-side` 是侧栏）。改造前这张卡的标题也叫 `.wb-title`，于是在独立版里被标题栏那条规则套上了 38px 高度与标题栏底色。
  - **不监听 scroll**，这是它与另外三只浮窗唯一的不同：那三只的目标是工程页里会滚动的行，页面一滚就得收；这个入口长在不滚的流水线条上（`.pipeline` 是 `flex: 0 0 auto`），消息流滚动时它一动不动。照抄那条会让「滚上去看前一轮对话，同时对着这一段的细纲」变得不可能——而那正是钉住要解决的情形。
- **四只浮窗共用一份定位**（`view/tip.ts` 的 `placeTip`）：挂 `body`、`position: fixed`、横向左对齐目标（右溢出往左收）、纵向优先下方（放不下翻上方、两边都不够就压 `max-height`）。从前这套几何在 `summaryTip` / `detailTip` / `errorTip` 里各抄了一遍。`minHeight: 0` 关掉压高度那一步，只有 `detailTip` 用它——那只浮窗只有一行、`pointer-events: none`、滚不动，压出来的 `max-height` 等于把字截掉。
...
- **右键一律接管**：全局监听里无条件 `preventDefault()`，输入框 / 文本域 / 内置编辑器里也一样，所以**原生的复制/粘贴/剪切菜单不会出现**。这是有意的取舍（菜单风格统一），代价是这几处的编辑项要自己实现（编辑器正文区与文件页已经做了，见 `editor/clipboard.ts` 与 `explorer/rows.ts`；输入框里目前只有「刷新」）。插件形态另需 [../src/shells/vscode/webviewHtml.ts](../src/shells/vscode/webviewHtml.ts) 的 `<body data-vscode-context='{"preventDefaultContextMenuItems": true}'>`：VS Code 给 webview 右键菜单加的复制/粘贴项由宿主渲染，JS 的 `preventDefault` 压不住，不加会同时冒出两层菜单。**监听挂在 `window` 的捕获阶段**，并额外接管副键的 `auxclick`：冒泡阶段的监听会被中途任何一处 `stopPropagation()` 挡掉，而触控板双指点击在部分浏览器/驱动上只发 `auxclick`、不发 `contextmenu`——两种漏法的现象都是「弹出原生右键菜单」。同一次点击的尾巴（落点没动、300ms 内）只挡不弹，免得一下点击弹两遍。`pointerdown` / `mousedown` 的默认行为**不碰**：按规范取消它们拦不住 `contextmenu`，却会把 `mousedown` 一起吃掉，于是文本域里右键不再挪光标，菜单里的「粘贴」会贴到上一次光标的位置去。触摸长按的原生 callout 走的不是 `contextmenu`，在 CSS 里用 `-webkit-touch-callout: none` 关（`css/view/base.css`）。
...
- **与「工程」页是两件事**：工程页是按语义整理过的视图（章节一条列表，带流水线状态与摘要新鲜度；角色带别名），只看得见几个可管理区里的文件；这一页是磁盘上真实的目录结构，一个都不藏。`.novelforge/` 里的摘要、会话、`project.json` 只有这里进得去——这就是它存在的理由，别为了「干净」把点开头的目录过滤掉（`fileTree.ts` 的 `HIDDEN_DIR_NAMES` 只挡 `node_modules` 与 `.git`）。
