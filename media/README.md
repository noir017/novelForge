...
- **命令类消息的气泡不能是空的**（`view/messages.ts` 的 `fillUserBody`）：命令（写剧情、拆成场景）一律不要求输入，作者一个字都不必打，于是 user 轮的 `content` 是空串。气泡里画一枚 `/写剧情` 标签（数据来自 `SerializedTurn.command`，后端记的是**按阶段具体化过的标签**），有补充要求就跟在下面。**命令名不写进 `content`**——那句话会被当成作者的要求装进 prompt，与旧界面逼他手打一句「请生成细纲」是同一个毛病。assistant 那一支**刻意保持成纯文本节点**：流式增量走 `body.textContent += delta`，里面有子元素的话第一片增量就会把它们冲掉。
- **要动磁盘之前那一句问画在气泡里**（`view/gate.ts`）：`gate` 到了就往这一轮的工具串（`.tools`）上挂一张卡片——要做什么、动的是哪个文件、这一次的参数（折叠着），加上按钮；点下去发回 `gateResult`。**两种问法共用这一张卡**：agent 动手前的闸门，以及**产物落盘前那一句**（后者从前是气泡末尾那颗「采纳写入」——一颗可以永远不点的按钮；它的拒绝那颗写「不采纳」而不是「跳过」，挂的位置也在正文**下面**，靠 `msg.callId` 在不在判断）。**不复用 `providerModal` 那个遮罩层**：作者要判断的上下文（它前几步读了什么、正要写哪个文件）就在被遮罩盖住的气泡里，而且模态框期间连翻一眼那个文件都做不到。**只有两颗按钮**——叫停整轮是输入框旁边那颗「停止」，不在这张卡上（摆进闸门只会被误当成「跳过」，而两者的后果差着一整轮）。按钮上的字全部来自消息（前端不写死），点过就地锁上（后端回话之前不锁的话，两颗都点得着，而只有第一下算数），答完收成一行留在原地。卡片不进 `SerializedTurn`——这一轮结束时气泡按会话重建，那时它自然消失，「作者跳过了这一步」已经记在工具条上了。
- **工作区卡是一只悬停浮窗**（`view/workbench.ts`）：流水线条上留一个一行高的入口（`#workbench`），悬停浮出这一层的产物，**点一下钉住**（照着场景素材写正文时鼠标要回输入框），Esc / 再点一次 / 点别处收起。从前它是消息流顶部一张 `position: sticky` 的卡片——滚不出视野，代价是永远占着消息流最上面那一块，而且**既关不掉也藏不起来**（只能折叠成一条标题栏，那条标题栏还在），侧边栏只有三百来像素宽，一份六小节的场景卡能吃掉半屏对话。改成浮窗之后 `renderSession` 不必再把卡片节点放回 `#messages` 了。
  ...
  - **不监听 scroll**，这是它与另外三只浮窗唯一的不同：那三只的目标是工程页里会滚动的行，页面一滚就得收；这个入口长在不滚的流水线条上（`.pipeline` 是 `flex: 0 0 auto`），消息流滚动时它一动不动。照抄那条会让「滚上去看前一轮对话，同时对着这一场的素材」变得不可能——而那正是钉住要解决的场景。
